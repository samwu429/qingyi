...
function buildSystemPrompt(): string {
  return [
    ...
    "5. income 单位是整数元；若截图是「万」请换算成元。粉丝数去掉逗号/「万」后换算为整数。",
...
// Run one admin assistant turn: multimodal user message + tool-calling loop.
// 执行一轮后台助手对话：多模态用户消息 + 工具调用循环。
export async function runAdminAssistant(options: {
  ...
  const model = hasImages ? groqConfig.visionModel : groqConfig.adminModel;
